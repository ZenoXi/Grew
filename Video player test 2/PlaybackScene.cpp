...
ID2D1Bitmap1* PlaybackScene::_Draw(Graphics g)
{
    ...
    (
        D2D1::SizeU(fd->GetWidth(), fd->GetHeight()),
        props,
        &frame
    );
    ...
    D2D1_RECT_F srcRect = D2D1::RectF(0.0f, 0.0f, fd->GetWidth(), fd->GetHeight());
    D2D1_RECT_F destRect;
    float tWidth = g.target->GetSize().width;
    float tHeight = g.target->GetSize().height;
    if (fd->GetWidth() / (float)fd->GetHeight() < tWidth / tHeight)
    {
        float scale = fd->GetHeight() / tHeight;
        float newWidth = fd->GetWidth() / scale;
        destRect = D2D1::Rect
        (
    ...
    else if (fd->GetWidth() / (float)fd->GetHeight() > tWidth / tHeight)
    {
        float scale = fd->GetWidth() / tWidth;
        float newHeight = fd->GetHeight() / scale;
        destRect = D2D1::Rect
        (
            0.0f,
